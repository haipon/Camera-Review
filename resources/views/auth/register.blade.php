<x-layout>
    <div class="pt-32">
        <form method="POST" action="{{ route('register') }}">
            @csrf

            <!-- Name -->
            <div class="mt-2 flex flex-col items-start justify-center">
                <x-input-label for="name" :value="__('Name')" class="ms-[12.5%]" />
                <x-text-input id="name" class="block mt-1 w-3/4 mx-auto" type="text" name="name" :value="old('name')" required autofocus autocomplete="name" />
                <x-input-error :messages="$errors->get('name')" class="mt-2 ms-[12.5%]" />
            </div>

            <!-- Email Address -->
            <div class="mt-4 flex flex-col items-start justify-center">
                <x-input-label for="email" :value="__('Email')" class="ms-[12.5%]" />
                <x-text-input id="email" class="block mt-1 w-3/4 mx-auto" type="email" name="email" :value="old('email')" required autocomplete="username" />
                <x-input-error :messages="$errors->get('email')" class="mt-2 ms-[12.5%]" />
            </div>

            <!-- Password -->
            <div class="mt-4 flex flex-col items-start justify-center">
                <x-input-label for="password" :value="__('Password')" class="ms-[12.5%]" />
                <x-text-input id="password" class="block mt-1 w-3/4 mx-auto" type="password" name="password" required autocomplete="new-password" />
                <x-input-error :messages="$errors->get('password')" class="mt-2 ms-[12.5%]" />
            </div>

            <!-- Confirm Password -->
            <div class="mt-4 flex flex-col items-start justify-center">
                <x-input-label for="password_confirmation" :value="__('Confirm Password')" class="ms-[12.5%]" />
                <x-text-input id="password_confirmation" class="block mt-1 w-3/4 mx-auto" type="password" name="password_confirmation" required autocomplete="new-password" />
                <x-input-error :messages="$errors->get('password_confirmation')" class="mt-2 ms-[12.5%]" />
            </div>

            <!-- Register button -->
            <div class="flex items-center justify-end mt-4 w-3/4 mx-auto">
                <a class="underline text-sm text-gray-600 hover:text-gray-900 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" href="{{ route('login') }}">
                    {{ __('Already registered?') }}
                </a>

                <x-primary-button class="ms-4">
                    {{ __('Register') }}
                </x-primary-button>
            </div>
        </form>
    </div>
</x-layout>
